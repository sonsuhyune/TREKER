...
class Transformer(nn.Module):
    ...
    def forward(self, batch_token_seqs, batch_token_masks, batch_token_types):
        ...
        if batch_num_tokens <= self.max_num_tokens:
            batch_output = self.transformer(input_ids=batch_token_seqs, attention_mask=batch_token_masks,
                                            token_type_ids=batch_token_types, output_attentions=True)
            ...
            for doc_id, real_num_token in enumerate(real_num_tokens):
                if real_num_token <= self.max_num_tokens:
                    ...
                    new_token_seq1 = torch.cat(
                        [batch_token_seqs[doc_id, :self.max_num_tokens - self.end_token_len], self.end_token_ids],
                        dim=-1)
                    new_token_mask1 = batch_token_masks[doc_id, :self.max_num_tokens]
                    new_token_type1 = torch.cat([batch_token_types[doc_id, :self.max_num_tokens - self.end_token_len],
                                                 batch_token_types[
                                                     doc_id, self.max_num_tokens - self.end_token_len - 1].repeat(
                                                     self.end_token_len)], dim=-1)

                    new_token_seq2 = torch.cat([self.start_token_ids, batch_token_seqs[doc_id,
                                                                      real_num_token - self.max_num_tokens + self.start_token_len: real_num_token]],
                                               dim=-1)
                    new_token_mask2 = batch_token_masks[doc_id, real_num_token - self.max_num_tokens: real_num_token]
                    new_token_type2 = torch.cat([batch_token_types[
                                                     doc_id, real_num_token - self.max_num_tokens + self.start_token_len].repeat(
                        self.start_token_len), batch_token_types[doc_id,
                                               real_num_token - self.max_num_tokens + self.start_token_len: real_num_token]],
                                                dim=-1)
            ...
            batch_token_seqs, batch_token_masks, batch_token_types = torch.stack(new_token_seqs,
                                                                                 dim=0).long(), torch.stack(
                new_token_masks, dim=0).float(), torch.stack(new_token_types, dim=0).long()
            batch_output = self.transformer(input_ids=batch_token_seqs, attention_mask=batch_token_masks,
                                            token_type_ids=batch_token_types, output_attentions=True)
            batch_token_embs, batch_token_atts = batch_output[0], batch_output[-1][-1]
            ...
            for (new_token_seq, real_num_token) in zip(new_token_segs, real_num_tokens):
                if new_token_seq == 1:
                    ...
                    new_token_att = F.pad(batch_token_atts[seg_id], (
                    0, batch_num_tokens - self.max_num_tokens, 0, batch_num_tokens - self.max_num_tokens))
                    ...
                    new_token_emb1 = F.pad(batch_token_embs[seg_id][:valid_num_token1],
                                           (0, 0, 0, batch_num_tokens - valid_num_token1))
                    new_token_mask1 = F.pad(batch_token_masks[seg_id][:valid_num_token1],
                                            (0, batch_num_tokens - valid_num_token1))
                    new_token_att1 = F.pad(batch_token_atts[seg_id][:, :valid_num_token1, :valid_num_token1], (
                    0, batch_num_tokens - valid_num_token1, 0, batch_num_tokens - valid_num_token1))

                    valid_num_token2 = real_num_token - self.max_num_tokens
                    new_token_emb2 = F.pad(batch_token_embs[seg_id + 1][self.start_token_len:], (
                    0, 0, valid_num_token2 + self.start_token_len, batch_num_tokens - real_num_token))
                    new_token_mask2 = F.pad(batch_token_masks[seg_id + 1][self.start_token_len:], (
                    valid_num_token2 + self.start_token_len, batch_num_tokens - real_num_token))
                    new_token_att2 = F.pad(
                        batch_token_atts[seg_id + 1][:, self.start_token_len:, self.start_token_len:], (
                        valid_num_token2 + self.start_token_len, batch_num_tokens - real_num_token,
                        valid_num_token2 + self.start_token_len, batch_num_tokens - real_num_token))
            ...
            # print(len(new_token_embs[0]))
            # print(len(new_token_atts[0][0]))
...
class Loss:
    ...
    def cal_PER_loss(self, batch_PER_reps, batch_epair_pooled_evidences):
        ...
        batch_PER_loss = F.binary_cross_entropy_with_logits(batch_PER_reps, batch_epair_pooled_evidences.clamp(max=1),
                                                            reduction='none')
    ...
    def cal_FER_loss(self, batch_FER_reps, batch_epair_finegrained_evidences):
        ...
        if batch_epair_finegrained_evidences is None:
            batch_FER_loss = 0
        else:
            batch_FER_loss = F.binary_cross_entropy_with_logits(batch_FER_reps, batch_epair_finegrained_evidences)
...
class Model(nn.Module):
    ...
    def get_epair_infos(self, batch_token_embs, batch_token_atts, batch_epair_tids, batch_epair_masks,
                        batch_num_epairs_per_doc):

        batch_epair_dids = torch.arange(batch_num_epairs_per_doc.shape[0], device = self.info.DEVICE_GPU).repeat_interleave(
            batch_num_epairs_per_doc).unsqueeze(-1).unsqueeze(-1)
        ...
        batch_epair_reps = [self.head_extractor_module(batch_epair_embs[:, 0, :]),
                            self.tail_extractor_module(batch_epair_embs[:, 1, :])]
        ...
        batch_epair_contexts = torch.cat(
            [doc_epair_atts @ batch_token_embs[did, :-1, :] for did, doc_epair_atts in enumerate(batch_epair_atts)])
    ...
    def get_sent_infos(self, batch_token_embs, batch_sent_tids, batch_num_sents_per_doc):
        ...
        batch_sent_dids = torch.arange(batch_num_sents_per_doc.shape[0], device = self.info.DEVICE_GPU).repeat_interleave(batch_num_sents_per_doc)
        batch_sent_embs = batch_token_embs[batch_sent_dids, batch_sent_tids]
        batch_sent_embs = batch_sent_embs.view(-1, batch_sent_embs.shape[-1] // self.args.bilinear_block_size,
                                               self.args.bilinear_block_size)
        batch_sent_embs = torch.split(batch_sent_embs, batch_num_sents_per_doc.tolist(),
                                      dim=0)  # doc: num_sent * num_block * size_block

        return batch_sent_embs

    def get_RE_reps(self, batch_epair_reps, batch_epair_contexts):
        # print("&& ", len(batch_epair_reps))
        ...
        batch_head_reps = batch_head_reps.view(-1, self.args.hidden_size // self.args.bilinear_block_size,
                                               self.args.bilinear_block_size)
        batch_tail_reps = batch_tail_reps.view(-1, self.args.hidden_size // self.args.bilinear_block_size,
                                               self.args.bilinear_block_size)
        batch_RE_reps = (batch_head_reps.unsqueeze(3) * batch_tail_reps.unsqueeze(2)).view(-1,
                                                                                           self.args.hidden_size * self.args.bilinear_block_size)
    ...
    def get_CR_reps(self, batch_token_embs, batch_mention_tids, batch_num_mentions_per_doc):
        ...
        for doc_token_embs, doc_mention_tids in zip(batch_token_embs, batch_mention_tids):
            ...
            doc_mention_embs = doc_mention_embs.view(-1, doc_mention_embs.shape[-1] // self.args.bilinear_block_size,
                                                     self.args.bilinear_block_size)  # num_mention * num_block * size_block
            doc_mention_reps = contract('mbi,nbj->mnbij', doc_mention_embs, doc_mention_embs).flatten(2, 4).flatten(0,
                                                                                                                    1)  # num_mention * num_mention * num_block * size_block * size_block
    ...
    def get_PER_reps(self, batch_epair_contexts, batch_num_epairs_per_doc, batch_sent_embs):
        ...
        batch_epair_contexts = batch_epair_contexts.view(-1, batch_epair_contexts.shape[
            -1] // self.args.bilinear_block_size, self.args.bilinear_block_size)
        batch_epair_contexts = torch.split(batch_epair_contexts, batch_num_epairs_per_doc.tolist(),
                                           dim=0)  # doc: num_epair * num_block * size_block
        batch_PER_reps = [contract('ebi,sbj->esbij', doc_epair_contexts, doc_sent_embs) for
                          doc_epair_contexts, doc_sent_embs in zip(batch_epair_contexts,
                                                                   batch_sent_embs)]  # doc: num_epair * num_sent * num_block * size_block * size_block
    ...
    def get_FER_reps(self, batch_epair_embs, batch_epair_contexts, batch_epair_relations, batch_num_epairs_per_doc,
                     batch_sent_embs):

        batch_epair_embs, batch_epair_contexts, batch_epair_relations = map(
            lambda x: torch.split(x, batch_num_epairs_per_doc.tolist(), dim=0),
            [batch_epair_embs, batch_epair_contexts, batch_epair_relations])

        batch_triplet_reps, batch_num_triplet_per_doc = [], []
        for doc_epair_embs, doc_epair_contexts, doc_epair_relations in zip(batch_epair_embs, batch_epair_contexts,
                                                                           batch_epair_relations):
            ...
            doc_epair_reps = torch.cat(
                [doc_epair_embs[:, 0, :], doc_epair_embs[:, 1, :], doc_epair_contexts[doc_epair_with_relations]], dim=1)
            doc_epair_reps = torch.cat(
                [epair_reps.expand(epair_num_relations, -1) for epair_reps, epair_num_relations in zip(doc_epair_reps,
                                                                                                       doc_epair_num_relations)]) if doc_epair_num_relations.sum() > 0 else doc_epair_reps.repeat(
                0, 1)
        ...
        batch_FER_reps = []
        # print("** ", len(batch_triplet_reps))
        for doc_triplet_reps, doc_sent_embs in zip(batch_triplet_reps, batch_sent_embs):
            doc_triplet_reps = doc_triplet_reps.view(-1, doc_triplet_reps.shape[-1] // self.args.bilinear_block_size,
                                                     self.args.bilinear_block_size)

            doc_FER_reps = contract('tbi,sbj->tsbij', doc_triplet_reps[:1000], doc_sent_embs).flatten(2, 4).flatten(0, 1)
            batch_FER_reps.append(doc_FER_reps)

        # if self.args.local_rank == 0:
        #     print(len(batch_FER_reps))
        #     print(len(batch_FER_reps[0]))
        #     print()
    ...
    def forward(self, batch_tasks, batch_inputs, to_evaluate=False, to_predict=False):
        ...
        batch_token_seqs, batch_token_masks, batch_token_types = batch_inputs['batch_token_seqs'], batch_inputs[
            'batch_token_masks'], batch_inputs['batch_token_types']
        batch_token_embs, batch_token_atts = self.transformer_module(batch_token_seqs, batch_token_masks,
                                                                     batch_token_types)

        batch_epair_tids, batch_epair_masks, batch_num_epairs_per_doc = batch_inputs['batch_epair_tids'], batch_inputs[
            'batch_epair_masks'], batch_inputs['batch_num_epairs_per_doc']
        batch_epair_embs, batch_epair_reps, batch_epair_contexts = self.get_epair_infos(batch_token_embs,
                                                                                        batch_token_atts,
                                                                                        batch_epair_tids,
                                                                                        batch_epair_masks,
                                                                                        batch_num_epairs_per_doc)
        ...
        if self.info.TASK_PER in batch_tasks or self.info.TASK_FER in batch_tasks:
            batch_sent_tids, batch_num_sents_per_doc = batch_inputs['batch_sent_tids'], batch_inputs[
                'batch_num_sents_per_doc']
        ...
        if self.info.TASK_CR in batch_tasks:
            batch_mention_tids, batch_mention_coreferences, batch_num_mentions_per_doc = batch_inputs[
                                                                                             'batch_mention_tids'], \
                                                                                         batch_inputs[
                                                                                             'batch_mention_coreferences'], \
                                                                                         batch_inputs[
                                                                                             'batch_num_mentions_per_doc']
            batch_CR_reps = self.get_CR_reps(batch_token_embs, batch_mention_tids, batch_num_mentions_per_doc)
            if to_evaluate: batch_loss += self.args.loss_weight_CR * self.loss_module.cal_CR_loss(batch_CR_reps,
                                                                                                  batch_mention_coreferences)
        ...
        if self.info.TASK_ET in batch_tasks:
            ...
            if to_evaluate: batch_loss += self.args.loss_weight_ET * self.loss_module.cal_ET_loss(batch_ET_reps,
                                                                                                  batch_epair_types)
        ...
        if self.info.TASK_PER in batch_tasks:
            ...
            if to_evaluate: batch_loss += self.args.loss_weight_PER * self.loss_module.cal_PER_loss(batch_PER_reps,
                                                                                                    batch_epair_pooled_evidences)

        if self.info.TASK_FER in batch_tasks:
            batch_epair_relations, batch_epair_finegrained_evidences = batch_inputs['batch_epair_relations'], \
                                                                       batch_inputs['batch_epair_finegrained_evidences']
            batch_FER_reps = self.get_FER_reps(batch_epair_embs, batch_epair_contexts, batch_epair_relations,
                                               batch_num_epairs_per_doc, batch_sent_embs)
            if to_evaluate: batch_loss += self.args.loss_weight_FER * self.loss_module.cal_FER_loss(batch_FER_reps,
                                                                                                    batch_epair_finegrained_evidences)
