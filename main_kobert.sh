...
save_name = "kobert_ALL"

python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=47030 main.py  --stage=${stage} --save_name = ${save_name} --dataset=${dataset} --seed=${seed} --transformer=${transformer} --hidden_size=${hidden_size} --bilinear_block_size=${bilinear_block_size} --RE_max=${RE_max} --CR_focal_gamma=${CR_focal_gamma} --PER_focal_gamma=${PER_focal_gamma} --FER_threshold=${FER_threshold} --loss_weight_CR=${loss_weight_CR} --loss_weight_ET=${loss_weight_ET} --loss_weight_PER=${loss_weight_PER} --loss_weight_FER=${loss_weight_FER} --num_epoch=${num_epoch} --batch_size=${batch_size} --update_freq=${update_freq} --new_lr=${new_lr} --pretrained_lr=${pretrained_lr} --warmup_ratio=${warmup_ratio} --max_grad_norm=${max_grad_norm}