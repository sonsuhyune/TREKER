...
from util import set_seed, myprint, prepare_autos, prepare_optimizer_scheduler, register_hyperparameters_main, cal_f1, \
    evaluate, prepare_batch_train, prepare_batch_test, feed_batch
import wandb
import os
...
def parse_args():
    ...
    parser.add_argument('--loss_weight_PER', type=float, default=0)
    parser.add_argument('--loss_weight_FER', type=float, default=0)
...
def train(args, info, idx_epoch, inputs_train, model, optimizer, scheduler):
    myprint(f'Start Training Epoch {idx_epoch}', info.FILE_STDOUT, args.local_rank)
    ...
    # # batch_tasks = set([info.TASK_RE, info.TASK_ET])
    # # batch_tasks = set([info.TASK_RE, info.TASK_CR])

    batch_task_list = [info.TASK_RE]
    if args.loss_weight_CR != 0:
        batch_task_list.append(info.TASK_CR)
    if args.loss_weight_ET != 0:
        batch_task_list.append(info.TASK_ET)


    batch_tasks = set(batch_task_list)
    print("#############     ", batch_tasks)
    # batch_tasks = set([info.TASK_RE])
    ...
    for idx_batch, batch_inputs in enumerate(prepare_batch_train(info, inputs_train, args.batch_size)):
        ...
        if idx_batch % report_batch == 0:
            myprint(
                f'Finish Training Epoch {idx_epoch} | Report Batch {idx_batch // report_batch} | Loss {batch_loss.item():.4f}',
...
def test(args, info, mode, inputs, tokenizer, model, if_final=False, rej_rate=0, all_shifts=None):
    myprint(f'Start Testing {mode}', info.FILE_STDOUT,args.local_rank)
    myprint('-' * 20, info.FILE_STDOUT,args.local_rank)
    print("len(inputs)",len(inputs))

    batch_tasks, batch_size, infer_round = set([info.TASK_RE]), args.batch_size, info.INFER_ROUND_FIRST
    myprint(f'Start {infer_round} Round of Inference', info.FILE_STDOUT,args.local_rank)
    model.eval()
    with torch.no_grad():
        ...
        for idx_batch, batch_inputs in enumerate(prepare_batch_test(info, inputs, batch_size, infer_round)):
            ...
            batch_triplets, batch_relations, batch_first_predictions = feed_batch(info, batch_inputs, batch_preds,
                                                                                  infer_round)
    ...
    if not if_final:
        all_first_P, all_first_R, all_first_F1 = cal_f1(all_relations,
                                                        model.module.loss_module.pred_RE_results(all_first_predictions))

        # all_first_F1_threshold = 0.62
        # if all_first_F1 <= all_first_F1_threshold:
        myprint(
            f'All {info.INFER_ROUND_FIRST} Precision: {all_first_P:.4f} | Recall: {all_first_R:.4f} | F1: {all_first_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        if args.local_rank == 0:
            wandb.log({
                "eval precision":all_first_P,
                "eval recall":all_first_R,
                "eval f1":all_first_F1,
            })
        return all_first_F1

    myprint(f'Identify Rejection Triplets', info.FILE_STDOUT,args.local_rank)
    ...
    with torch.no_grad():
        ...
        for idx_batch, batch_inputs in enumerate(
                prepare_batch_test(info, inputs, batch_size, infer_round, preds=rej_triplets_eids2rid2sids)):
            _, batch_preds = model(batch_tasks, batch_inputs, to_evaluate=False, to_predict=True)
            batch_FER_triplets, batch_FER_predictions = feed_batch(info, batch_inputs, batch_preds, infer_round)
            rej_FER_triplets += batch_FER_triplets;
            rej_FER_predictions += batch_FER_predictions
    ...
    del rej_FER_triplets
    del rej_FER_predictions
    torch.cuda.empty_cache()

    batch_tasks, batch_size, infer_round = set([info.TASK_RE]), 1, info.INFER_ROUND_MASK
    myprint(f'Start {infer_round} Round of Inference', info.FILE_STDOUT,args.local_rank)
    model.eval()
    with torch.no_grad():
        ...
        for idx_batch, batch_inputs in enumerate(
            ...
            rej_mask_triplets += batch_mask_triplets;
            rej_mask_predictions += batch_mask_predictions

    corpus = json.load(open(info.FILE_CORPUSES[mode], 'r'))
    corpus_title2idx = {doc['title']: did for did, doc in enumerate(corpus)}
    myprint(f'Construct FER Docs for Rejection Triplets', info.FILE_STDOUT,args.local_rank)
    ...
    for FER_title, eids2rids in rej_triplets_FER2eids2rids.items():
        ...
        if doc_title not in corpus_title2idx.keys():
            continue
        ...
        for sid, sent in enumerate(doc['sents']):
            ...
            sid2tids[sid_] = (len(doc_tokens), len(doc_tokens) + len(sent_tokens));
            sid_ += 1
    ...
    batch_tasks, batch_size, infer_round = set([info.TASK_RE]), 1, info.INFER_ROUND_DOC
    myprint(f'Start {infer_round} Round of Inference', info.FILE_STDOUT,args.local_rank)
    model.eval()
    with torch.no_grad():
        ...
        for idx_batch, batch_inputs in enumerate(
                prepare_batch_test(info, rej_triplets_FER_docs, batch_size, infer_round)):
            _, batch_preds = model(batch_tasks, batch_inputs, to_evaluate=False, to_predict=True)
            batch_doc_triplets, batch_doc_predictions = feed_batch(info, batch_inputs, batch_preds, infer_round)
            rej_doc_triplets += batch_doc_triplets;
            rej_doc_predictions += batch_doc_predictions

    myprint(f'Blend All Rounds of Inference', info.FILE_STDOUT,args.local_rank)
    # print(rej_mask_triplets)
    ...
    count_total = 0
    count = 0
    # print("all_triplets",len(all_triplets))
    for idx, ((doc_title, eid_i, eid_j), mask) in enumerate(zip(all_triplets, rej_triplet_masks)):
        for rid in mask.nonzero().flatten().tolist():
            count_total += 1
            triplet = (doc_title, eid_i, eid_j, rid)

            if triplet not in rej_mask_triplet2idx.keys():
                count += 1
                continue
            all_predictions[1, idx, rid] = rej_mask_predictions[rej_mask_triplet2idx[triplet]]
            all_predictions[2, idx, rid] = rej_doc_predictions[
                rej_doc_triplet2idx[triplet]] if triplet in rej_doc_triplet2idx else 0
    # print(count)
    # print(count_total)
    # print("-"*50)
    ...
    def eval():
        loss = F.binary_cross_entropy_with_logits(blend(all_shifts, all_predictions, rej_triplet_masks),
                                                  all_relations[rej_triplet_masks])
        optimizer.zero_grad()
        loss.backward()
        return loss

    all_relations, all_predictions, rej_triplet_masks = all_relations.float().to(info.DEVICE_GPU), all_predictions.to(
        info.DEVICE_GPU), rej_triplet_masks.to(info.DEVICE_GPU)
    # print("len(all_relations)",len(all_relations))
    ...
    if not if_final:
        ...
        rej_first_P, rej_first_R, rej_first_F1 = cal_f1(all_relations[rej_triplet_masks],
                                                        all_predictions[0][rej_triplet_masks].gt(0))
        rej_mask_P, rej_mask_R, rej_mask_F1 = cal_f1(all_relations[rej_triplet_masks],
                                                     all_predictions[1][rej_triplet_masks].gt(0))
        rej_doc_P, rej_doc_R, rej_doc_F1 = cal_f1(all_relations[rej_triplet_masks],
                                                  all_predictions[2][rej_triplet_masks].gt(0))
        rej_blend_P, rej_blend_R, rej_blend_F1 = cal_f1(all_relations[rej_triplet_masks],
                                                        all_predictions[3][rej_triplet_masks].gt(0))
        myprint(
            f'Rej {info.INFER_ROUND_FIRST} Precision: {rej_first_P:.4f} | Recall: {rej_first_R:.4f} | F1: {rej_first_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        myprint(
            f'Rej {info.INFER_ROUND_MASK} Precision: {rej_mask_P:.4f} | Recall: {rej_mask_R:.4f} | F1: {rej_mask_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        myprint(
            f'Rej {info.INFER_ROUND_DOC} Precision: {rej_doc_P:.4f} | Recall: {rej_doc_R:.4f} | F1: {rej_doc_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        myprint(
            f'Rej {info.INFER_ROUND_BLEND} Precision: {rej_blend_P:.4f} | Recall: {rej_blend_R:.4f} | F1: {rej_blend_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)

        all_mask_P, all_mask_R, all_mask_F1 = cal_f1(all_relations,
                                                     model.module.loss_module.pred_RE_results(all_predictions[1]))
        all_doc_P, all_doc_R, all_doc_F1 = cal_f1(all_relations, model.module.loss_module.pred_RE_results(all_predictions[2]))
        all_blend_P, all_blend_R, all_blend_F1 = cal_f1(all_relations,
                                                        model.module.loss_module.pred_RE_results(all_predictions[3]))
        myprint(
            f'All {info.INFER_ROUND_FIRST} Precision: {all_first_P:.4f} | Recall: {all_first_R:.4f} | F1: {all_first_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        myprint(
            f'All {info.INFER_ROUND_MASK} Precision: {all_mask_P:.4f} | Recall: {all_mask_R:.4f} | F1: {all_mask_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        myprint(
            f'All {info.INFER_ROUND_DOC} Precision: {all_doc_P:.4f} | Recall: {all_doc_R:.4f} | F1: {all_doc_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        myprint(
            f'All {info.INFER_ROUND_BLEND} Precision: {all_blend_P:.4f} | Recall: {all_blend_R:.4f} | F1: {all_blend_F1:.4f}',
            info.FILE_STDOUT,args.local_rank)
        ...
        all_blend_predictions = model.module.loss_module.pred_RE_results(all_predictions[3])
        all_triplets_eids2rid2sids = defaultdict(lambda: defaultdict(dict))
        # print("all_blend_predictions", len(all_blend_predictions))
        # print("all_triplets", len(all_triplets))
        ...
        with torch.no_grad():
            ...
            for idx_batch, batch_inputs in enumerate(
                    prepare_batch_test(info, inputs, batch_size, infer_round, preds=all_triplets_eids2rid2sids)):
                _, batch_preds = model(batch_tasks, batch_inputs, to_evaluate=False, to_predict=True)
                # print(idx_batch,"batch_inputs", len(batch_inputs))
                # print(idx_batch,"batch_preds", len(batch_preds))
                batch_FER_triplets, batch_FER_predictions = feed_batch(info, batch_inputs, batch_preds, infer_round)
                # print("idx_batch, len(batch_FER_triplets)",idx_batch, len(batch_FER_triplets))
                # print("idx_batch, len(batch_FER_predictions)",idx_batch, len(batch_FER_predictions))
                all_FER_triplets += batch_FER_triplets
                all_FER_predictions += batch_FER_predictions

        myprint('-' * 20, info.FILE_STDOUT,args.local_rank)
        # print("all_FER_triplets, all_FER_predictions")
        # print(len(all_FER_triplets))
        # print(len(all_FER_predictions))
...
def main():
    ...
    if args.local_rank == 0:
        ...
        wandb.init(project="naver_docRE",
            reinit=True, config=args)
    # wandb.config.update(args)

    myprint('=' * 20, info.FILE_STDOUT,args.local_rank)
    myprint(f'Start {args.stage} {args.dataset}', info.FILE_STDOUT,args.local_rank)
    myprint('-' * 20, info.FILE_STDOUT,args.local_rank)

    register_hyperparameters_main(args, info)

    myprint('-' * 20, info.FILE_STDOUT,args.local_rank)
    myprint('Initialize Relevant Objects', info.FILE_STDOUT, args.local_rank)
    myprint('-' * 20, info.FILE_STDOUT,args.local_rank)

    set_seed(args.seed)
    rtype_embeddings = torch.from_numpy(np.load(info.DATA_REL2VEC)).float()
    inputs_train, inputs_dev, inputs_test = map(lambda x: pk.load(open(info.FILE_INPUTS[x], 'rb')),
                                                [info.MODE_TRAIN, info.MODE_DEV, info.MODE_TEST])
    ...
    model = Model(args, info, transformer, rtype_embeddings).to(device)
    ...
    rej_rate, all_shifts = test(args, info, info.MODE_DEV, inputs_dev, tokenizer, model, if_final=True, rej_rate=0,
                                all_shifts=None)
    test(args, info, info.MODE_TEST, inputs_test, tokenizer, model, if_final=True, rej_rate=rej_rate,
         all_shifts=all_shifts)

    myprint(f'Finish {args.stage} {args.dataset}', info.FILE_STDOUT,args.local_rank)
    myprint('=' * 20, info.FILE_STDOUT,args.local_rank)
