...
def prepare_optimizer_scheduler(args, num_train_docs, model):
    ...
    grouped_lrs = [{'params': grouped_parameters[group], 'lr': lr} for group, lr in
                   zip(['pretrained_lr', 'new_lr'], [args.pretrained_lr, args.new_lr])]
...
def evaluate(args, info, mode, triplets, sids):
    myprint(f'Start Evaluating {mode} Result', info.FILE_STDOUT,args.local_rank)
    ...
    for triplet, sid in zip(triplets, sids):
        outputs.append(
            {'title': triplet[0], 'h_idx': triplet[1], 't_idx': triplet[2], 'r': info.DATA_ID2REL[triplet[3]],
             'evidence': sid.gt(args.FER_threshold).nonzero().flatten().tolist()})

    json.dump(outputs, open(f'{info.FILE_RESULTS[mode]}.json', 'w',encoding="utf-8"),ensure_ascii=False)
    ...
    if mode == info.MODE_DEV:
        ...
        for output in outputs:
            ...
            if output_key in truths:
                ...
                # print(output['evidence'])
                # print(truths[output_key])
                Correct_sent += len(output['evidence'] and truths[output_key])

            if output_key not in dev_in_train:
                if output_key in truths:
                    Correct_ign_train += 1
                else:
                    Incorrect_ign_train += 1

        Precision = Correct / Pred if Pred != 0 else 0
        Recall = Correct / len(truths)
        F1 = 2 * Precision * Recall / (Precision + Recall) if Precision + Recall != 0 else 0
        myprint(f'{mode} Set - Base Precision: {Precision:.4f} | Recall: {Recall:.4f} | F1: {F1:.4f}', info.FILE_STDOUT,args.local_rank)

        Precision_ign_train = Correct_ign_train / (
                    Correct_ign_train + Incorrect_ign_train) if Correct_ign_train + Incorrect_ign_train != 0 else 0
        Recall_ign_train = Correct_ign_train / (len(truths) - len(dev_in_train))
        F1_ign_train = 2 * Precision_ign_train * Recall_ign_train / (
                    Precision_ign_train + Recall_ign_train) if Precision_ign_train + Recall_ign_train != 0 else 0
        myprint(
            f'{mode} Set - Ignore Train Precision: {Precision_ign_train:.4f} | Recall: {Recall_ign_train:.4f} | F1: {F1_ign_train:.4f}',
        ...
        F1_sent = 2 * Precision_sent * Recall_sent / (
                    Precision_sent + Recall_sent) if Precision_sent + Recall_sent != 0 else 0
        myprint(
            f'{mode} Set - Evidence Precision: {Precision_sent:.4f} | Recall: {Recall_sent:.4f} | F1: {F1_sent:.4f}',
            info.FILE_STDOUT,args.local_rank)
...
def register_hyperparameters_prepare(args, info):
    myprint(f'Transformer = {args.transformer}', info.FILE_STDOUT,0)
    myprint(f'Max Sequence Length = {args.max_seq_length}', info.FILE_STDOUT,0)
    # myprint(f'Transformer = {args.transformer}', info.FILE_STDOUT,args.local_rank)
    # myprint(f'Max Sequence Length = {args.max_seq_length}', info.FILE_STDOUT,args.local_rank)


def register_hyperparameters_main(args, info):
    myprint(f'Seed = {args.seed}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Transformer = {args.transformer}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Hidden Dim = {args.hidden_size}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Bilinear Block Size = {args.bilinear_block_size}', info.FILE_STDOUT,args.local_rank)

    myprint(f'RE Max Relations = {args.RE_max}', info.FILE_STDOUT,args.local_rank)
    myprint(f'CR Focal Gamma = {args.CR_focal_gamma}', info.FILE_STDOUT,args.local_rank)
    myprint(f'PER Focal Gamma = {args.PER_focal_gamma}', info.FILE_STDOUT,args.local_rank)
    myprint(f'FER Threshold = {args.FER_threshold}', info.FILE_STDOUT,args.local_rank)

    myprint(f'CR Loss Weight = {args.loss_weight_CR}', info.FILE_STDOUT,args.local_rank)
    myprint(f'ET Loss Weight = {args.loss_weight_ET}', info.FILE_STDOUT,args.local_rank)
    myprint(f'PER Loss Weight = {args.loss_weight_PER}', info.FILE_STDOUT,args.local_rank)
    myprint(f'FER Loss Weight = {args.loss_weight_FER}', info.FILE_STDOUT,args.local_rank)

    myprint(f'Num of Epoch = {args.num_epoch}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Batch Size = {args.batch_size}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Updating Frequency = {args.update_freq}', info.FILE_STDOUT,args.local_rank)

    myprint(f'New Learning Rate = {args.new_lr}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Pretrained Learning Rate = {args.pretrained_lr}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Warmup Ratio = {args.warmup_ratio}', info.FILE_STDOUT,args.local_rank)
    myprint(f'Max Gradient Norm = {args.max_grad_norm}', info.FILE_STDOUT,args.local_rank)
    ...
    for idx_batch in range(num_batch):
        ...
        for doc_input in batch_inputs:
            ...
            if doc_input == None:
                doc_input_none += 1
                continue
            # print(len(doc_input.eid2tids.values()))
            if len(doc_input.eid2tids.values()) == 0:
                continue
            ...
            for mid in range(len(doc_mention_tids) - 1):
                doc_mention_coreferences[doc_mention_tids[mid]:doc_mention_tids[mid + 1],
                doc_mention_tids[mid]:doc_mention_tids[mid + 1]] = 1
            ...
            # print(len(doc_epair_tids))
            if len(doc_epair_tids) == 0: continue
            ...
            if len(doc_epair_finegrained_evidences) != 0: batch_epair_finegrained_evidences.append(
                torch.cat(doc_epair_finegrained_evidences))
            batch_num_epairs_per_doc.append(doc_num_epairs_per_doc)
        # print(batch_token_seqs)
        # print(len(batch_token_seqs))
        if len(batch_token_seqs) == 0: continue
        batch_token_seqs = rnn.pad_sequence(batch_token_seqs, batch_first=True, padding_value=0).long().to(
            info.DEVICE_GPU)
        batch_token_masks = rnn.pad_sequence(batch_token_masks, batch_first=True, padding_value=0).float().to(
            info.DEVICE_GPU)
        batch_token_types = rnn.pad_sequence(batch_token_types, batch_first=True, padding_value=0).long().to(
            info.DEVICE_GPU)
        ...
        # print(batch_epair_tids[0].size())
        if len(batch_epair_tids) == 0: continue
        max_epair_tids = max([doc_epair_tids.shape[-1] for doc_epair_tids in batch_epair_tids])
        batch_epair_tids = torch.cat(
            [F.pad(doc_epair_tids, (0, max_epair_tids - doc_epair_tids.shape[-1]), value=-1) for doc_epair_tids in
             batch_epair_tids]).long().to(info.DEVICE_GPU)
        batch_epair_types = torch.cat(batch_epair_types).long().to(info.DEVICE_GPU)
        batch_epair_masks = torch.cat(
            [F.pad(doc_epair_masks, (0, batch_token_seqs.shape[-1] - doc_epair_masks.shape[-1]), value=0) for
             doc_epair_masks in batch_epair_masks]).float().to(info.DEVICE_GPU)
        ...
        batch_epair_finegrained_evidences = torch.cat(batch_epair_finegrained_evidences).float().to(
            info.DEVICE_GPU) if len(batch_epair_finegrained_evidences) != 0 else None
...
def prepare_batch_test(info, inputs, batch_size, infer_round, preds=None):
    ...
    # doc_titles = doc_titles[:10]
    num_batch = math.ceil(len(doc_titles) / batch_size)
    # print(num_batch) #1589
    ...
    for idx_batch in range(num_batch):
        ...
        for doc_title, doc_input in zip(batch_titles, batch_inputs):
            ...
            if infer_round == info.INFER_ROUND_FIRST:
                epair2rids = [[epair, list(doc_input.eids2rid2sids[epair].keys())] for epair in
                              permutations(doc_input.eid2etype, 2)]
            ...
            for epair, rids in epair2rids:
                ...
                epair_tids = torch.full((2, max_epair_tids), -1)
                epair_tids[0, :len(doc_input.eid2tids[epair[0]])] = torch.Tensor(
                    list(doc_input.eid2tids[epair[0]])).long()
                epair_tids[1, :len(doc_input.eid2tids[epair[1]])] = torch.Tensor(
                    list(doc_input.eid2tids[epair[1]])).long()
                doc_epair_tids.append(epair_tids)
                ...
                for rid in rids:
                    epair_relations[rid] = 1
                # print("epair_relations", epair_relations)
                doc_epair_relations.append(epair_relations)
            if doc_epair_tids == []:
                count += 1
                continue
        ...
        max_epair_tids = [doc_epair_tids.shape[-1] for doc_epair_tids in batch_epair_tids]
        # print(num_batch, idx_batch, max_epair_tids)
        if len(max_epair_tids) == 0: continue
        max_epair_tids = max(max_epair_tids)
        # max_epair_tids = max([doc_epair_tids.shape[-1] for doc_epair_tids in batch_epair_tids])
        batch_epair_tids = torch.cat(
            [F.pad(doc_epair_tids, (0, max_epair_tids - doc_epair_tids.shape[-1]), value=-1) for doc_epair_tids in
             batch_epair_tids]).long().to(info.DEVICE_GPU)
...
def feed_batch(info, batch_inputs, batch_preds, infer_round):
    ...
    batch_num_epairs_per_doc, batch_num_sents_per_doc = batch_inputs['batch_num_epairs_per_doc'].tolist(), batch_inputs[
        'batch_num_sents_per_doc']
    batch_epair_relations, batch_preds = batch_inputs['batch_epair_relations'].to(info.DEVICE_CPU), batch_preds.to(
        info.DEVICE_CPU)
    # print("===========")
    # print(batch_epair_relations)
    # print("===========")
    ...
    if infer_round != info.INFER_ROUND_FIRST:
        ...
        if infer_round == info.INFER_ROUND_FER:
            ...
            batch_preds = torch.split(batch_preds, batch_num_epairs_per_doc, dim=0)
